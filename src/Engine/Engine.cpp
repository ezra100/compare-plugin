...
int getBestMatchingSubChunkOffset(const chunk_info* shorterChunk, const chunk_info* longerChunk)
{
	...
	for (int step = endLine, line = 0, bestMatchWordsCount = 0; line < endLine && line != bestMatchingSubChunkOffset;)
	{
		// Sub-chunk to test
		...
		diff_info& blockDiff1, diff_info& blockDiff2)
{
	...
	*pStartLine2 = getBestMatchingSubChunkOffset(pChunk1, pChunk2);
	...
	for (int line1 = 0; line1 < pChunk1->lineCount; ++line1)
	{
		...
		// Make sure that enough portion of the line is matched
...
void markSection(const diff_info& bd, const DocCmpInfo& doc)
{
	...
	for (int i = doc.section.off, line = bd.off + doc.section.off; i < endOff; ++i, ++line)
	{
		const int markerMask =
				(bd.isMoved(i) == NOT_MOVED) ? doc.blockDiffMark :
...
bool markAllDiffs(CompareInfo& cmpInfo, AlignmentInfo_t& alignmentInfo)
{
	...
	for (int i = 0; i < blockDiffSize; ++i)
	{
		...
		{
			pMainAlignData->diffMark	= 0;
			pSubAlignData->diffMark		= 0;
		...
		else if (bd.type == diff_type::DIFF_IN_2)
		{
			...
			pMainAlignData->diffMark	= 0;
			pSubAlignData->diffMark		= cmpInfo.doc2.blockDiffMark;
		...
		else if (bd.type == diff_type::DIFF_IN_1)
		{
			if (bd.matchedDiff)
			{
				...
				for (int j = 0; j < changedLinesCount; ++j)
				{
					...
					{
						pMainAlignData->diffMark	= cmpInfo.doc1.blockDiffMark;
						pSubAlignData->diffMark		= cmpInfo.doc2.blockDiffMark;
					...
					pMainAlignData->diffMark	= MARKER_MASK_CHANGED;
					pSubAlignData->diffMark		= MARKER_MASK_CHANGED;
		...
		const UserSettings& settings, AlignmentInfo_t& alignmentInfo)
{
	...
	{
		cmpInfo.doc1.blockDiffMark = MARKER_MASK_REMOVED;
		cmpInfo.doc2.blockDiffMark = MARKER_MASK_ADDED;
	}
	else
	{
		cmpInfo.doc1.blockDiffMark = MARKER_MASK_ADDED;
		cmpInfo.doc2.blockDiffMark = MARKER_MASK_REMOVED;
	}
